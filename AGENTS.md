...
- Load `.agents/skills/nltps-mps-completion/SKILL.md` for completion, assurance,
  evidence, or acceptance work.
- The controlled MPS project is `mps/NLTPSGovernance`. Its language stack is
  `nltps.foundation`, `nltps.governance`, `nltps.clinicalintent`,
  `nltps.roles.*`, and `nltps.realization`; `nltps.corpus` is the 119-HLR
  Stage A mirror and `nltps.modeltests` contains semantic model tests.
- Stage A is materially complete but not formally closed. Do not invent new
  language work merely to make progress. Classify every remaining item as an
  executable observation, an engineering decision, or a named independent
  human review obligation before acting.
- The completion coordinator may implement and verify, but it may not approve
  its own work, supply a human reviewer identity, amend frozen acceptance
  criteria, or declare Stage A/Stage C authority.
- Run `python tools/repo/check_workspace_location.py --enforce` before any MPS
  write. Model persistence is changed only with `mps_mcp_*` tools.
- Generation and semantic validation are separate claims. Use
  `tools/mps/headless_build.py --cold --target make` and
  `tools/mps/headless_build.py --target test` as separate observations and
  require a nonzero discovered test population.
