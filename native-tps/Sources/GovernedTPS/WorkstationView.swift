                        ...
                        if let artifact = store.selectedArtifact {
                            HStack {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    Text(artifact.operation.title+" · "+(artifact.modelID.hasPrefix("matRad/") ? "matRad generic photon plan" : artifact.isDemo ? "analytic fixture" : artifact.modelID.hasPrefix("learned-gaussian-contour/") ? "learned Gaussian baseline" : "local Core ML")).font(.system(size: 12, weight: .medium))
