...
\addlinespace[2pt]
C1 & Operational & Corrected by relocation & Closed by establishing the engineering repository as an independent clone outside synchronized storage and verifying 14 of 14 gates from it. Never waived and never accepted. \\
\addlinespace[2pt]
...
C1 is closed by relocation, never by waiver. While the remedy was outstanding it was dispositioned \texttt{BLOCKED\_ENVIRONMENT}: a known environmental prerequisite, not a model exception, and at no point waived, accepted, or granted a permitted-failure allowance. The gate remained mandatory throughout.
...
A linked worktree relocates the working tree only; its object database, refs, worktree metadata, locks, and index state remain under the controlling common directory. That was sufficient to demonstrate the working-tree gate but was not the configuration chosen for persistent MPS authoring. The managed worktree therefore served only to publish the baseline, and the engineering repository is now an independent clone whose Git common directory sits outside synchronized storage.

Relocation verification at commit \texttt{7fc8217}, tag \texttt{gcpl-mps-stage-a-pre-materialization}, recorded in \nolinkurl{spec/relocation_verification.yaml}: 14 of 14 gates green, 26 of 26 tests passing, 2,144 records, 2,144 verification claims, zero identifier loss, zero hazard-set alteration, 76 of 2,144 source-explicit, 2,068 derived, zero CRLF text files, and byte-identical controlled artifacts across the move. The prior synchronized checkout is retained as a read-only historical location, still fails the gate by design, and is not an engineering workspace.
