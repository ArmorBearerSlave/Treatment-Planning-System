...
\addlinespace[2pt]
C1 & Operational & Managed worktree prepared; not executed & A strict location gate and managed Git worktree procedure exist; the current checkout remains affected. \\
\addlinespace[2pt]
...
The active checkout is still inside OneDrive and its path contains whitespace. \nolinkurl{tools/repo/check_workspace_location.py --enforce} rejects that condition. \nolinkurl{scripts/create_mps_worktree.ps1} requires a clean worktree, refuses an existing, synchronized, or whitespace-bearing destination, creates or attaches the dedicated \texttt{mps/materialization} branch, verifies the shared Git common directory and source commit, and does not delete the source. Worktree creation is intentionally not executed while corrective changes are uncommitted.
...
\bottomrule
\endlastfoot
\nolinkurl{python -m unittest discover -s tests -v} & Tooling regressions pass; this is not clinical V\&V. \\
\addlinespace[2pt]
\nolinkurl{python tools/mps/build_hlr_import_bundle.py --check} & The 119-HLR Stage A mirror is schema-valid and deterministic. \\
\addlinespace[2pt]
\nolinkurl{python tools/spec/build_trace_graph.py --check} & All 2,144 entities and claim IDs have a materialized hazard allocation. \\
\addlinespace[2pt]
\nolinkurl{python tools/spec/build_risk_register.py --check --gate 0} & The 2,088 risk records are hazard-derived and structurally consistent. \\
\addlinespace[2pt]
\nolinkurl{python tools/spec/check_controlled_specs.py} & Required specs, counts, schemas, and baselines reconcile. \\
\addlinespace[2pt]
\nolinkurl{python tools/spec/check_gate_readiness.py --gate 0} & Nonclinical structural work may proceed within the frozen architecture. \\
\addlinespace[2pt]
\nolinkurl{python tools/spec/check_gate_readiness.py --gate 1} & Expected to fail until named owners approve all required risk scores and quality targets. \\
\end{longtable}
\end{small}
