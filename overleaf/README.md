...
This directory contains four independent LaTeX documents derived from the
...
All four documents use `nl_tps_common.sty`. The ConOps also uses the images in
`figures/`. For reliable compilation from the repository root, use these
top-level Overleaf entry points:

- `main.tex` - default combined review copy containing all four documents
...
2. The default `main.tex` compiles all four documents in trace order: ConOps,
   High-Level Requirements, Detailed Sub-Requirements, and Component
   Realization. To compile one controlled document by itself,
   open **Settings -> Compiler -> Main document** and select
   `NL_TPS_ConOps_Overleaf.tex`,
   `NL_TPS_High_Level_Requirements_Overleaf.tex`, or
...
The four files remain separate controlled documents. The combined suite is a
