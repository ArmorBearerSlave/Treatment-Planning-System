...
This directory contains five independent LaTeX documents derived from the
...
All five documents use `nl_tps_common.sty`. The ConOps also uses the images in
...
2. The default `main.tex` compiles all five documents in trace order: ConOps,
   High-Level Requirements, High-Level Interface Requirements, Detailed
   Sub-Requirements, and Component Realization. To compile one controlled document by itself,
...
The five files remain separate controlled documents. The combined suite is a
